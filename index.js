...
// --- INICIALIZACIÓN DE VARIABLES GLOBALES DE BULMA BOT ---
global.owners = global.owners || []
global.rentaTimers = global.rentaTimers || {}
global.tempGroupList = global.tempGroupList || []
// ---------------------------------------------------------
...
console.log(chalk.bold.cyanBright(`\n✰ Iniciando 𝐁𝐔𝐋𝐌𝐀 𝐁𝐔𝐋𝐌𝐀 𝐁𝐎𝐓 🧬 ✰\n`))

cfonts.say('BULMA BOT', {
  font: 'block',
  align: 'center',
  colors: ['cyanBright'],
})

cfonts.say(`Developed By • @𝐀𝐥𝐚𝐧 𝐒𝐡𝐨𝐩`, {
  font: 'console',
  align: 'center',
  colors: ['blueBright'],
})
...
if (!methodCodeQR && !methodCode && !fs.existsSync(`${SESSIONS_DIR}/creds.json`)) {
  do {
    opcion = await question(
      colores('⌨ Seleccione una opción de vinculación:\n') +
...
const nameqr = global.botname || 'Bulma Bot'
  ...
  if (opcion === '2' || methodCode) {
    ...
    if (!global.conn.authState.creds.registered) {
      ...
      setTimeout(async () => {
        ...
        console.log(chalk.bold.white(chalk.bgCyan(`✧ CÓDIGO DE VINCULACIÓN ✧`)), chalk.bold.white(codeBot))
      }, 3000)
    }
  }
}
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    console.log(chalk.bold.green('\n❀ ¡𝐁𝐔𝐋𝐌𝐀 𝐁𝐔𝐋𝐌𝐀 𝐁𝐎𝐓 Conectado con éxito! ❀'))
  }
...
const pluginFolder = path.resolve(__dirname, 'commands')
...
global.reload = async (filename) => {
  try {
    ...
    filename = filename.replace(/^commands\//, '')
...
export async function filesInit(folder = 'commands') {
...
await filesInit('commands')
console.log(`🌙 ${Object.keys(global.plugins).length} plugins cargados con éxito.`)
